...
from airbnb_listings.items import AirBnBListingItem, AirBnbId, ExpandedAirBnBListingItem
...
MAP_URL_TEMPLATE3 = "https://www.airbnb.ca/s/Vancouver--Canada/homes?drawer_open=true&tab_id=home_tab&refinement_paths%5B%5D=%2Fhomes&query=Vancouver%2C%20Canada&place_id=ChIJs0-pQ_FzhlQRi_OBm-qWkbs&flexible_trip_lengths%5B%5D=one_month&monthly_start_date=2024-05-27&monthly_length=2&monthly_end_date=2024-07-01&search_mode=regular_search&price_filter_input_type=0&channel=EXPLORE&ne_lat={}&ne_lng={}&sw_lat={}&sw_lng={}&search_by_map=true&zoom_level={}&map_toggle=true&search_type=user_map_move&price_filter_num_nights=28&zoom={}"
...
class UpdateListingSpider(scrapy.Spider):
    """
    Spider for scraping Airbnb listings.
    """
    name = "update_listing_spider"
        ...
        try:
            with open(
                ...
                for row in read_csv_file:
                    row: dict
                    UpdateListingSpider.ALREADY_SCRAPED_LISTINGS.add(row['airbnb_listing_id'])
                print("ids", UpdateListingSpider.ALREADY_SCRAPED_LISTINGS)
        except FileNotFoundError as e:
            print(e)
        try:
            with open('cordinates.json', 'r') as file:
                coordinates = json.load(file)
                print("cord", len(coordinates))
                if coordinates is not None:
                    random.shuffle(coordinates)
                    return [scrapy.FormRequest(
                        url.format(cord["ne_lat"], cord["ne_lng"], cord["sw_lat"], cord["sw_lng"], ZOOM_LEVEL,
                                   ZOOM_LEVEL)) for url in
                        MAP_URL_TEMPLATES for
                        cord in
                        coordinates]
    ...
    async def parse(self, response: Response, **kwargs):
        ...
        if UpdateListingSpider.next_page_cursors is None:
            UpdateListingSpider.next_page_cursors = self._get_cursors(script_json)
        for result in results:
            ...
            if listing_id is not None and listing_id not in UpdateListingSpider.ALREADY_SCRAPED_LISTINGS:
                try:
        ...
        if len(UpdateListingSpider.next_page_cursors) != 0:
            cursor_id = UpdateListingSpider.next_page_cursors.pop()
    ...
    def handle_listing(response):
        ...
        person_capacity = ""
        try:
            metadata = UpdateListingSpider._get_metadata_from_json(script_tag_json)
        ...
        number_of_baths = ""
        try:
            sections = UpdateListingSpider._parse_listing_json(script_tag_json)
                ...
                if section.get("sectionComponentType") == "PDP_DESCRIPTION_MODAL":
                    ...
                    for item in items:
                        if item.get("title") == "Registration number":
                            registration_number = item.get("html", "Not Found").get(
                                "htmlText", "Not Found")
        ...
        listing_item["baths"] = number_of_baths
    ...
    def _parse_listings_json(listings_json):
        ...
        """
        return UpdateListingSpider._safe_get(listings_json,
    ...
    def _parse_listing_json(listing_json):
        ...
        """
        return UpdateListingSpider._safe_get(listing_json, "data", "presentation",
    ...
    def _get_cursors(script_json):
        ...
        """
        return UpdateListingSpider._safe_get(script_json,
                                             "niobeMinimalClientData", 0, 1, "data", "presentation", "staysSearch",
